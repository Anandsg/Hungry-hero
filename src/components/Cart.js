...
const Cart = ()=>{
    ...
    return(
        <div className="md:w-3/5 w-[80%]  m-auto py-4 min-h-screen ">
            ...
            <button className="text-xs font-medium bg-red-300 py-1 px-2 hover:bg-red-400 transition-all duration-300 ease-in-out rounded" onClick={()=>HandleClearCart()}>Clear Cart</button>
            </div>
...
const CartItem = ({name,imageId,price,description})=>{
    return(
        <div className="my-2 border-b py-2 md:flex-row flex flex-col text-sm  gap-8">
            <img
                  src={
                   imageId ? (CDN_URL + imageId) : "https://res.cloudinary.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_508,h_320,c_fill/39cd5e4824e5c011ffaf56ddc39891e8"
                  }
                  alt=""
